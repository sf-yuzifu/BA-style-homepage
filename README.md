...
全站内容（站点信息、联系方式、项目展示、音乐列表、Live2D 角色等）均可通过根目录的 `_config.yaml` 一个文件完成定制，无需修改源码即可部署属于你自己的主页。
...
本项目所有个性化内容均通过根目录下的 **`_config.yaml`** 完成配置（YAML 格式，方便阅读；如需从旧版 JSON 迁移，可通过 [此网站](https://www.json.cn/json2yaml/) 快速转换）。
...
本项目支持多语言国际化。配置采用「**基础配置 + 语言包覆盖**」的两层结构：
